    ...
    class DatasetIterator:
        ...
        def init_dataset(self):
            ...
            while len(self.queue) == 0:
                time.sleep(1)
            self.dataset = StateActionReturnDataset.from_config(self.queue.get(block=True), self.context_length)
        ...
        def __iter__(self):
            ...
            if self.producer is None:
                self.producer = []
                for i in range(5):
                    self.producer.append(torch.multiprocessing.Process(target=producer, args=(self.config, rng, self.queue, False, self.context_length))) # config, np.RNG, queue, verbose
                    self.producer[i].start()
